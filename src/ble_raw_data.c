 ...
 *
 * Architecture:
 *   IMU callback → queue (never touches BLE)
 *   k_work_delayable drains queue → bt_gatt_notify
 */
...
static bool s_enabled; /* mode gating; drives both notify output and advertising */
...
static void start_raw_adv(void) {
    ...
    if (err == 0) {
        DBG_PRINTK("RAW_ADV: started\n");
    } else if (err != -EALREADY) {
        DBG_PRINTK("RAW_ADV: failed %d\n", err);
    }
}

static void raw_adv_work_fn(struct k_work* work) {
    ARG_UNUSED(work);
    start_raw_adv();
}
...
void ble_raw_data_set_enabled(bool enabled) {
    s_enabled = enabled;
    DBG_PRINTK("STATUS:BLE_RAW:%s\n", enabled ? "on" : "off");

    if (s_adv_raw) {
        bt_le_ext_adv_stop(s_adv_raw);
    }
    if (s_adv_raw && enabled) {
        start_raw_adv();
    }
}
...
#define QUEUE_CAP  256u /* ~5 s at 52 Hz; power-of-2 for fast masking */
#define QUEUE_MASK (QUEUE_CAP - 1u)

static struct imu_notify_payload s_queue[QUEUE_CAP];
static uint32_t                  s_queue_head; /* written by IMU callback only */
static uint32_t                  s_queue_tail; /* read/written by drain work only */
...
static void drain_work_fn(struct k_work* work) {
    ...
    struct imu_notify_payload sample = s_queue[s_queue_tail & QUEUE_MASK];
...
static void on_imu_sample(const struct imu_sample* s) {
    ...
    if ((s_queue_head - s_queue_tail) >= QUEUE_CAP) {
        s_queue_tail++;
    }

    s_queue[s_queue_head & QUEUE_MASK] = (struct imu_notify_payload){
        .ts_ticks = s->ts_ticks,
        .ax = s->ax,
        .ay = s->ay,
        .az = s->az,
...
void ble_raw_data_init(void) {
    ...
    if (id < 0 && id != -ENOMEM) {
        DBG_PRINTK("RAW_ID_CREATE_ERR: %d\n", id);
    }
    ...
    if (err) {
        DBG_PRINTK("RAW_ADV_CREATE_ERR: %d\n", err);
        return;
    }
...
void ble_raw_data_set_enabled(bool e) {
    (void)e;
}

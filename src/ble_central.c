 ...
 * For demo purposes primarily, to show multi-role BLE works with other functionality.
 */
...
static void handle_command(const uint8_t* data, uint16_t len) {
    for (size_t i = 0; i < ARRAY_SIZE(commands); i++) {
        ...
        if (len == cmd_len && memcmp(data, commands[i], cmd_len) == 0) {
            ...
            ARG_UNUSED(state);
            DBG_PRINTK("CENTRAL_CMD: %s -> LED%u %s\n", commands[i], (unsigned)(i + 1),
                       state == 0 ? "toggled" : "toggle failed");
            return;
        }
    }
    DBG_PRINTK("CENTRAL_CMD: unrecognized (%u bytes)\n", len);
}
...
static uint8_t notify_func(struct bt_conn*                  conn,
    ...
    if (!data) {
        DBG_PRINTK("CENTRAL: unsubscribed\n");
...
static uint8_t discover_func(struct bt_conn*                 conn,
    ...
    if (!attr) {
        DBG_PRINTK("CENTRAL: discovery failed/complete\n");
    ...
    if (!bt_uuid_cmp(discover_params.uuid, &esp32_svc_uuid.uuid)) {
        ...
        if (err) {
            DBG_PRINTK("CENTRAL: characteristic discover failed (%d)\n", err);
        }
        ...
        err = bt_gatt_discover(conn, &discover_params);
        if (err) {
            DBG_PRINTK("CENTRAL: CCC discover failed (%d)\n", err);
        }
    } else {
        ...
        if (err && err != -EALREADY) {
            DBG_PRINTK("CENTRAL: subscribe failed (%d)\n", err);
        } else {
            DBG_PRINTK("CENTRAL: subscribed\n");
        }
        return BT_GATT_ITER_STOP;
    }
...
static void start_discovery(struct bt_conn* conn) {
    ...
    if (err) {
        DBG_PRINTK("CENTRAL: service discover failed (%d)\n", err);
    }
}
...
static void device_found(const bt_addr_le_t* addr, int8_t rssi, uint8_t type,
    ...
    DBG_PRINTK("CENTRAL: ESP32 found (%s), connecting\n", addr_str);
    ...
    if (err) {
        DBG_PRINTK("CENTRAL: create conn failed (%d)\n", err);
        start_scan();
    }
}
...
static void start_scan(void) {
    ...
    if (err == 0) {
        DBG_PRINTK("CENTRAL: scanning for ESP32\n");
    } else if (err != -EALREADY) {
        DBG_PRINTK("CENTRAL: scan start failed (%d)\n", err);
    }
}
...
static void central_connected(struct bt_conn* conn, uint8_t err) {
    ...
    if (err) {
        DBG_PRINTK("CENTRAL: connect failed (%u)\n", err);
    ...
    DBG_PRINTK("CENTRAL: connected to ESP32\n");
...
static void central_disconnected(struct bt_conn* conn, uint8_t reason) {
    ...
    DBG_PRINTK("CENTRAL: disconnected from ESP32 (reason 0x%02x)\n", reason);
    bt_conn_unref(esp32_conn);
    esp32_conn = NULL;
    start_scan();
}
...
void ble_central_init(void) {
    for (size_t i = 0; i < ARRAY_SIZE(cmd_leds); i++) {
        if (!gpio_is_ready_dt(&cmd_leds[i])) {
            DBG_PRINTK("CENTRAL: LED%u not ready\n", (unsigned)(i + 1));
            continue;
        }
